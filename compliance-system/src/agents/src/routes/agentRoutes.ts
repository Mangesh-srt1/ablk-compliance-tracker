...
router.post(
  ...
  [
    ...
    body('transaction.type')
      .isString()
      .isIn(['transfer', 'trade', 'withdrawal', 'deposit', 'security'])
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result: ComplianceResult =
        await agents.orchestrator.executeComplianceCheck(transaction);
...
router.post(
  ...
  [
    body('transactions')
      ...
      .withMessage('Transactions array is required (1-100 items)'),
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const results: ComplianceResult[] =
        await agents.orchestrator.executeBatchComplianceChecks(transactions);
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result: ComplianceResult | null =
        await agents.orchestrator.getComplianceStatus(transactionId);
