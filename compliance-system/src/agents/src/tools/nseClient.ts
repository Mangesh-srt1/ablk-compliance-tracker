/**
 * NSE Client
 * Integration with National Stock Exchange
 */
...
export class NSEClient {
  ...
  constructor() {
    ...
    this.baseUrl = process.env.NSE_BASE_URL || 'https://api.nseindia.com/v1';
  ...
  async getQuote(symbol: string): Promise<any> {
    try {
      logger.info('Getting NSE quote', { symbol });
  ...
  async getMarketDepth(symbol: string): Promise<any> {
    try {
      logger.info('Getting NSE market depth', { symbol });
