...
router.post('/kyc-check',
  ...
  [
    ...
    body('documents.*.type')
      .isIn(['aadhaar', 'passport', 'drivers_license', 'national_id', 'utility_bill', 'bank_statement'])
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (error instanceof Error && 'httpStatus' in error) {
        ...
        return res.status(appError.httpStatus).json(createErrorResponseFromDetails(
          appError.code,
          appError.category,
          appError.message,
          appError.httpStatus,
...
router.get('/kyc-check/:checkId',
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!result) {
        return res.status(404).json(createErrorResponseFromDetails(
